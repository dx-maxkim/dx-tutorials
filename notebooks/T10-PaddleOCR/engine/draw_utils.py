...
def create_font(text, target_size, font_path=None):
    ...
    if chinese_font_path and Path(chinese_font_path).exists():
        min_font_size = max(24, min(target_size) // 3)  # Much larger minimum
...
def create_font_vertical(text, target_size, font_path=None):
    ...
    if chinese_font_path and Path(chinese_font_path).exists():
        ...
        min_font_size = max(24, min(char_height // 2, target_size[0] // 2))
...
def draw_box_txt_fine(img_size, box, txt, font_path=None):
    ...
    text_width = max(box_width - 3 * padding, 16)
    text_height = max(box_height - 3 * padding, 16)
...
def draw_with_poly_enhanced(
        image,
        bbox_text_poly_shape_quadruplets,
        font_path=None,
        use_precise_transform=True
        ):
